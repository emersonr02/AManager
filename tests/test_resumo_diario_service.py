...
def ambiente_resumo(tmp_path, monkeypatch):
    """ProducaoService e MaquinaService já foram migrados para SQLite —
    isola uma BD temporária com o esquema criado."""
...
def _seed_logs(ambiente, logs):
    """Insere produções de teste diretamente em SQLite. Aceita os mesmos
    dicts "soltos" usados antes com JSON (id/maquina/data_inicio/estado/
    tempo_real/tempo_estimado/nc_codigo/acoes_aplicadas) — maquina_id
    fica sempre NULL aqui (os testes deste ficheiro não dependem da
    resolução via parque de máquinas para a própria produção, só para o
    dict `id_para_nome`, que vem de MaquinaService de forma independente)."""
